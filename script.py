...
class SeparatePage():
    ...
    def separe_double_page_en_deux(self, image):
        ...
        bottompoint = [bottompoints[min(bottompoints, key=int)], min(
            bottompoints, key=int)]
        if bottompoint[1] == toppoint[1]:
            raise Exception('separation double page y=0', 'Impossible')
        self.OUTPUT.print("separation double page angle", np.arctan2(
            bottompoint[1] - toppoint[1], bottompoint[0] - toppoint[0])/np.pi*180)
        self.OUTPUT.print('separation double page y=0', (
            bottompoint[0]*toppoint[1]-toppoint[0]*bottompoint[1])/(toppoint[1]-bottompoint[1]))

        # On défini le masque pour séparer la page droite et gauche
        mask = np.zeros((h, w), np.uint8)
        pts = np.array([[0, 0], [toppoint[0], 0], [toppoint[0], toppoint[1]], [
            bottompoint[0], bottompoint[1]], [bottompoint[0], h-1], [0, h-1]])
    ...
    def remets_droit_la_page(self, image, n):
        ...
        angles = list(map(lambda line: np.arctan2(
    ...
    def isole_partie_interessante(self, image, n):
        ...
        for _, cnt in enumerate(contours):
            if 0.001*0.001*imgh*imgw < cv2.contourArea(cnt) and cv2.contourArea(cnt) < 0.5*imgh*0.5*imgw:
        ...
        return page_gauche_0, np.maximum(min_x-10, 0), np.minimum(max_x+10, imgw-1), np.maximum(min_y-10, 0), np.minimum(max_y+10, imgh-1), imgw, imgh
    ...
    def recadre(self, image, n, width_paper, height_paper, min_x, max_x, min_y, max_y, imgw, imgh):
        ...
        if marge_gauche_px+w+marge_droite_px < width_paper/2.54*dpi:
            ...
            image_recadre = cv2.copyMakeBorder(image, 0, 0, int(
                pixels_manquant/2.), int(pixels_manquant/2.), cv2.BORDER_CONSTANT, value=white)
        else:
        ...
        if marge_haute_px+h+marge_basse_px < height_paper/2.54*dpi:
            ...
            image_recadre2 = cv2.copyMakeBorder(image_recadre, int(
                pixels_manquant*pourcenthaut), int(pixels_manquant*pourcentbas), 0, 0, cv2.BORDER_CONSTANT, value=white)
        else:
            raise Exception('marge', 'marge_gauche_px')
    ...
    def treat_file(self, filename, dict_test = None):
        ...
        image1a2, min_x1, max_x1, min_y1, max_y1, imgw1, imgh1 = self.isole_partie_interessante(
            image1a, 1)
        image1b = self.crop_rectangle(image1a2, min_x1, max_x1, min_y1, max_y1)
        image2a2, min_x2, max_x2, min_y2, max_y2, imgw2, imgh2 = self.isole_partie_interessante(
            image2a, 2)
        ...
        image1c = self.recadre(image1b, 1, 21., 29.7, min_x1,
                        max_x1, min_y1, max_y1, imgw1, imgh1)
        image2c = self.recadre(image2b, 2, 21., 29.7, min_x2,
                        max_x2, min_y2, max_y2, imgw2, imgh2)
